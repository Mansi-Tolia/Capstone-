...
# Linear regression on best df version
```{r warning=FALSE}
#split 80/20
...
mean((train$default-predsLinTrain)^2)
mean((test$default-predsLinTest)^2)

#ridge
fitRidge <- linearRidge(default~., train)
predsRidgeTrain <- predict(fitRidge, train)
predsRidgeTest <- predict(fitRidge, test)
mean((train$default-predsRidgeTrain)^2)
mean((test$default-predsRidgeTest)^2)
...
while (length(xnames) > 0) {
  ...
  for (xname in xnames) {
    ...
    mse_train_tmp <- mean((train$default - yhat_train_tmp) ^ 2)
    
    # compute MSE test
    yhat_test_tmp <- predict(fit_fw_tmp, test)
    mse_test_tmp <- mean((test$default - yhat_test_tmp) ^ 2)
...
mean((train$default-predsTrain)^2)
mean((test$default-predsTest)^2)
```
