...
#Add Mansi clustering, get rid of `default` outliers
```{r message=FALSE, warning=FALSE}
df_mean <- read_csv('~/Desktop/DFs/df_mean.csv')
cluster <- read_csv('~/Desktop/DFs/df_clusters.csv') %>% select(cluster)
#df_mean <- add_column(df_mean, cluster = cluster$cluster, .after = names[length(names)-1])

iqrDefault <- IQR(df_mean$default)
df_mean <- df_mean[df_mean$default <= (mean(df_mean$default)+iqrDefault), ]
```


# Linear regression on best df_mean
```{r warning=FALSE}
#split 80/20
...
```{r}
years_index <- 2008:2015
train <- subset(df_mean, year>=2008 & year <= 2015)
test <- subset(df_mean, year>2015)

fitTime <- lm(default~., train)
predsTimeTrain <- predict(fitTime, train)
predsTimeTest <- predict(fitTime, test)
...
xnames <- xnames[!xnames %in% c("unitid", "default")]

#start with the intercept model
fit_fw <- lm(default ~ 1, data = train)

#predict the yhats based off fit_fw
yhat_train <- predict(fit_fw, train)
yhat_test <- predict(fit_fw, test)

#calculate MSEs
mse_train <- mean((train$default - yhat_train)^2, na.rm = T)
mse_test <- mean((test$default - yhat_test)^2, na.rm = T)

#log the intercept only model to match against
xname <- "intercept"
log_fw <-tibble(
  xname = xname,
...
# an outer while loop considers all potential predictors until all of them
...
while (length(xnames) > 0) {
  ...
  for (xname in xnames) {
    ...
    mse_train_tmp <- mean((train$default - yhat_train_tmp)^2, na.rm = T)
    
    # compute MSE test
    yhat_test_tmp <- predict(fit_fw_tmp, test)
    mse_test_tmp <- mean((test$default - yhat_test_tmp)^2, na.rm = T)
...
#linear model
fit <- lm(formula = formula, train)
predsTrain <- predict(fit, train)
predsTest <- predict(fit, test)
...
```{r}
#save parameters
params <- as.data.frame(coef(fitTime))
...
grouped$logsafety <- log(grouped$safety)
```

```{r}
cor(grouped)
ggplot(grouped, aes(x = safety, y = cinsoff, color=safety))+
  geom_point()
ggplot(grouped, aes(x = logsafety, y = cinsoff, color=safety))+
  geom_point()
```
...
```{r}
df_clusters <- read_csv('~/Desktop/DFs/df_alldatawithclusters.csv')

df_clusters <- df_clusters %>% 
  group_by(unitid) %>%
  summarise_each(mean)

df_clusters$cluster<-round(df_clusters$cluster)

df_all_clusters<-inner_join(grouped,df_clusters)

df_all_clusters$cluster<-factor(df_all_clusters$cluster)

ggplot(df_all_clusters, aes(cluster, safety, group = cluster, color = cluster))+
  geom_boxplot()
  ggtitle("safety")
  
ggplot(df_all_clusters, aes(x = safety, y = cinsoff, col = cluster))+
  geom_point()
ggplot(df_all_clusters, aes(x = safety, y = cotsoff, col = cluster))+ 
geom_point()
