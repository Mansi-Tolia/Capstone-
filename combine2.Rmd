...
#split
trainTime <- subset(df_cluster, year>=2008 & year <= 2015)
testTime <- subset(df_cluster, year>2015)

#linear model
fitTimeTop20 <- lm(formula, trainTime)
predsTimeTrain <- predict(fitTimeTop20, trainTime)
predsTimeTest <- predict(fitTimeTop20, testTime)
mean((trainTime$default-predsTimeTrain)^2, na.rm = T)
mean((testTime$default-predsTimeTest)^2, na.rm = T)
```
...
```{r message=FALSE, warning=FALSE}
testTime$preds <- predsTimeTest

testTime <- df_cluster %>% 
  group_by(unitid) %>%
  summarise_each(mean)

df_cluster$cluster <- factor(round(df_cluster$cluster))
df_cluster$safety <- safety_log$safety


ggplot(testTime, aes(cluster, preds, group = cluster, color = cluster))+
  geom_boxplot()+
  labs(title = "Risk by cluster", x = "Cluster", y = "Risk")
  
ggplot(testTime, aes(x = preds, y = cinsoff, col = cluster))+
  geom_point()

ggplot(testTime, aes(x = preds, y = cotsoff, col = cluster))+ 
geom_point()
...
testTime %>% select_if(is.numeric) %>% cor() %>% View()
```
