...
cluster <- read_csv('~/Desktop/df_clusters.csv') %>% select(cluster)
...
```{r warning=FALSE}
#split 80/20
index <- sample(1:nrow(xvars), 0.8*nrow(xvars))
train <- df_mean[index, ]
test <- df_mean[-index, ]
...
#calculate risk score for each school
risk_log <- data.frame(unitid=numeric(),
                       risk = numeric())
...
for (i in 1:nrow(grouped)){
  ...
  risk <- risk + sum(tmp$product)
  
  #add to log
  risk_log <- risk_log %>% add_row(unitid = name, risk = risk)
}

#add back to risk
grouped$risk <- risk_log$risk
grouped$logRisk <- log(risk_log$risk)
```

```{r}
cor(grouped)
ggplot(grouped, aes(x = risk, y = cinsoff, col = cluster))+
  geom_point()
ggplot(grouped, aes(x = logRisk, y = cinsoff, col = cluster))+
  geom_point()
```
