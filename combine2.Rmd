...
#appl_sch_n to zeroes
combined_variable$appl_sch_n[is.na(combined_variable$appl_sch_n)] <-0
...
combined_variable$loan_ever <- NULL
combined_variable$par_ed_n <- NULL
...
combined_variable %>%
  ...
  impute_by_year(median) -> impute_medians

combined_variable %>% 
  select(-c(names(impute_medians)), unitid, year) %>%
  left_join(impute_medians, by = c("unitid", "year")) -> combined_variable

write_csv(combined_variable, '~/Desktop/combined_variable.csv')
